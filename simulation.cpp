...
int main(){
    ...
    int n; // number of points in disk
    ...
    int mstep = 100;    // number of steps to take

    // disk particle data
    // read or pass in values
    double m[n];
    double x[n], y[n], z[n];
    double vx[n], vy[n], vz[n];

    // log positions to plot later
    double xlog_nucleus[mstep], ylog_nucleusp[mstep], zlog_nucleus[mstep];
    double xlog_intruder[mstep], ylog_intruder[mstep], zlog_intruder[mstep];
